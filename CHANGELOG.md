...
- **Installation updates are easier to recover.** An update without
  ...
  command that rebinds it. The owner's JSON indentation is preserved. `check`
  reports a retained owner customization of an owned file as healthy instead
  of looping on an action-needed FIX, and any other action-needed `check`
