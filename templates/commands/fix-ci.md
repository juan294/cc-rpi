...
This legacy command is a rename notice only. Its workflow is now `rpi-fix-ci`.

- Claude direct installation: invoke `/rpi-fix-ci`.
- Claude plugin installation: invoke `/cc-rpi:rpi-fix-ci`.
- Codex direct installation: invoke `$rpi-fix-ci` or select the skill.
