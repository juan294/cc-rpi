...
| `/triage` | Discovers overnight agent reports via timestamp-based scanning, checks for agent failures in logs, synthesizes findings, proposes action plan for all items, implements fixes. Public repos: reports stay local. Private repos: reports are committed alongside fixes. | Every morning. First command of the day for each project. |
