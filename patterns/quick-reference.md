...
34. Diagnose HTTP 403 before changing access strategy -> skills/shell-tools
...
6. Reconcile remote integration changes locally before publication -> skills/git-workflow
7. Preserve and integrate work before removing its worktree -> skills/git-workflow
8. Inspect worktree ownership and dirty files before cleanup -> skills/git-workflow
15. Use safe branch deletion after verified integration -> skills/git-workflow
...
35. Handle gh checks pending exit 8 and verify expected inventory -> skills/github-cli
...
82. Check code-scanning product eligibility and caller permissions -> skills/github-cli
...
11. Inspect Node module format and generated bytes on CLI failure -> skills/ci-workflow
...
63. Batch verified dependency updates into local integration -> skills/deployment-safety
64. Every CI run costs money -- count first -> skills/deployment-safety
65. Run local runtime and platform packaging preflights -> skills/deployment-safety
